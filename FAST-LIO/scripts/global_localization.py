...
def global_localization(pose_estimation):
    ...
    # 粗配准
    # transformation, fitness = registration_at_scale(scan_tobe_mapped, global_map_in_FOV, initial=pose_estimation, scale=5)
    # print("Initial transformation",coarse_icp.fitness,np.linalg.norm(tf.transformations.translation_from_matrix(coarse_icp.transformation)))

    # 精配准
    transformation, fitness = registration_at_scale(scan_tobe_mapped, global_map_in_FOV, initial=pose_estimation, scale=1)
    ...
    if fitness > LOCALIZATION_TH:
        ...
        # print(fitness,np.linalg.norm(xyz))
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('/lio_odom', Odometry, cb_save_cur_odom, queue_size=1)

    # 初始化全局地图
    rospy.logwarn('Waiting for global map......')
    initialize_global_map(rospy.wait_for_message('/accumulated_cloud', PointCloud2))
    ...
    while not initialized:
        ...
        initial_odom = rospy.wait_for_message('/lio_odom', Odometry)
        initial_pose = pose_to_mat(initial_odom)
        if cur_scan:
